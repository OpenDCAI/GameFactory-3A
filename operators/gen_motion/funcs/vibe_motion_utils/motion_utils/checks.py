"""Summarize bone, ground, contact and target errors for motion clips."""
...
def metrics(clip, plan, *, contacts=None, targets=None) -> dict[str, object]:
    ...
    report = validate_clip(clip, plan, collision_stride=1)
    failures = [f.check for f in report.failures if f.check != 'self_collision']
    failures.extend(f.check for f in report.failures if f.check == 'self_collision')
