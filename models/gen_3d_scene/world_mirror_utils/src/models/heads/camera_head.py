...
from models.gen_3d_scene.world_mirror_utils.src.models.layers import Mlp
...
class CameraHead(nn.Module):
    ...
    def forward(self, feat_seq: list, steps: int = 4) -> list:
        ...
        for step in range(steps):
            ...
            if curr_pred is None:
                net_input = self.param_embed(
                    self.init_token.expand(b, seq_len, -1)
                )
            else:
                curr_pred = curr_pred.detach()
                net_input = self.param_embed(curr_pred)
            norm_shift, norm_scale, norm_gate = self.adapt_norm_gen(
                net_input
            ).chunk(3, dim=-1)
            mod_cam_feat = norm_gate * self.apply_adaptive_modulation(
                self.adapt_norm(cam_tokens), norm_shift, norm_scale
            )
    ...
    def apply_camera_parameter_activation(
    ...
    def apply_parameter_activation(
        self, tensor: torch.Tensor, act_type: str
    ) -> torch.Tensor:
    ...
    def apply_inverse_logarithm_transform(
        self, x: torch.Tensor
    ) -> torch.Tensor:
        """
