...
# AAAGameForge: gsplat is only needed for the 3D-Gaussian branch. Mesh
# generation uses the depth / pointmap / normal heads only, so keep the import
# soft and fail at call time instead of import time.
try:
    from gsplat.rendering import rasterization
    from gsplat.strategy import DefaultStrategy
except ImportError as _gsplat_error:  # pragma: no cover - depends on install

    def rasterization(*args, **kwargs):
        raise RuntimeError(
            "gsplat is required for the Gaussian-splatting branch. Install it with "
            "`pip install gsplat`, or keep WorldMirrorModel(enable_gs=False)."
        ) from _gsplat_error

    class DefaultStrategy:  # placeholder so class-level defaults still resolve
        pass

from models.gen_3d_scene.world_mirror_utils.src.models.utils.frustum import (
    calculate_unprojected_mask,
)
from models.gen_3d_scene.world_mirror_utils.src.models.utils.geometry import (
    depth_to_world_coords_points,
)
from models.gen_3d_scene.world_mirror_utils.src.models.utils import sh_utils, act_gs
    ...
    def rasterize_batches(
        ...
        **kwargs,
        ...
        for i in range(batch_size):
            ...
            render_colors_i, render_depths_i, render_alphas_i = (
...
class GaussianSplatRenderer(nn.Module):
    def __init__(
        ...
        for out_channel, s, b in splits_and_inits:
            nn.init.xavier_uniform_(
                final_conv_layer.weight[
                    start_channels : start_channels + out_channel
                ],
                s,
            )
            nn.init.constant_(
                final_conv_layer.bias[
                    start_channels : start_channels + out_channel
                ],
                b,
            )
    ...
    def render(
        ...
        predictions: Dict[
            str, torch.Tensor
        ],  # From WorldMirror: pose/depth/pts3d etc
        ...
        S = context_predictions.get("imgs", images).shape[
            1
        ]  # context view nums
        ...
        if self.training:
            ...
            unproject_masks = calculate_unprojected_mask(
                views, S
            )  # [B, V, H, W]
            valid_masks = torch.cat(
                [gt_valid_masks_src, (gt_valid_masks_tgt & unproject_masks)],
                dim=1,
            )
        else:
            ...
            if "camera_poses" in context_predictions:
                pred_context_extrinsic, _ = self.prepare_cameras(
                    context_predictions, S
                )
                scale_factor = pred_context_extrinsic[:, :, :3, 3].norm(
                    dim=-1
        ...
        if self.enable_conf_filter and "gs_depth_conf" in predictions:
            splats = self.apply_confidence_filter(
                splats, predictions["gs_depth_conf"]
            )
        ...
        for i in range(0, gt_colors.shape[1], chunk_size):
            ...
            rendered_colors, rendered_depths, rendered_alphas = (
                self.rasterizer.rasterize_batches(
                    ...
                    sh_degree=(
                        min(self.sh_degree, 0) if "sh" in splats else None
                    ),
                )
            )
    ...
    def apply_confidence_filter(self, splats, gs_depth_conf):
        """Apply confidence filtering to Gaussian splats before pruning. Discard bottom p%
        confidence points, keep top (100-p)%.
    ...
    def prune_gs(self, splats, voxel_size=0.002):
        ...
        for i in range(B):
            ...
            merged["sh"] = merged["sh"] / weight_sums.unsqueeze(-1).unsqueeze(
                -1
            )

            # Merge opacities (weighted sum of squares)
            merged["opacities"].scatter_add_(
                0, inverse_indices, weights * weights
            )
            ...
            merged["quats"] = merged["quats"] / torch.clamp(
                quat_norms, min=1e-8
            )
    ...
    def prepare_splats(
        ...
        splats["quats"] = act_gs.reg_dense_rotation(
            quats.reshape(B, S * H * W, 4)
        )
        ...
        splats["weights"] = act_gs.reg_dense_weights(
            weights.reshape(B, S * H * W)
        )
...
if __name__ == "__main__":
    ...
    colors, alphas, _ = rasterizer.rasterize_splats(
        splats, viewmats, Ks, width, height
    )
